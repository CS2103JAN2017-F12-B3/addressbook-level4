...
|1| edit title in task:`Write English Essay` | edit 45 Write Chinese Essay| Task is edited |
|2| edit end date in task:`Prepare for maths test` | edit 3 e/next year | Task is edited with its position in the sorted list changed to index 29 |
|3| edit multiple fields in task `Buy airtickets to maldives` | edit 47 d/with alice t/work | Task description and tag is editted |
|4| edit to remove fields in task `Buy airtickets to maldives` | edit 47 d/ t/ | Task description and tag are removed |
|5| edit a task that is done| list done | Selected view is now `Done` |
|| |  edit 1 t/random | The first task's tag in `Done` tab is edited |
|6| edit task using alias command | change 1 d/with boss | Task description is edited |
|Done|		|		|		|
|| **done task** | | Tasks that are done will be moved from `To Do` tab to `Done` tab |
|1| mark task `Write chinese essay` as done | list todo | Selected view is `To Do` |
|| | done 45 | Task is marked done |
|2| mark recurring task `Buy Lunch` as done | done 38 | A copy of task marked as done is created while the repeating task's start date advances by one day and is still a to do task |
|3| mark last occurence of recurring task `Cut hair` as done | done 6 | Task is marked done and there are no more recurrences of the task |
...
|| | done 1 | Error message will be shown |
|Undone|		|		|		|
|| **undone task**<br><br>Tasks that are undone will be moved from `Done` tab to `To Do` tab | | |
|1| mark task `Write chinese essay` as undone in `Done` tab | undone 14 | Task is marked incomplete|
|2| mark task `Cut hair` as undone | undone 13 | task is marked undone, does not add back the recurrence pattern it used to have |
...
|| | undone 1 | Error message will be shown |
|Select|		|		|		|
|| **select task** | | list will automatically scroll to and highlight the selected task |
|1| select `Buy Lunch` | select 38 | Task with given index is selected |
|2| select invalid index | select 1000 | Error message will be shown |
|Delete|		|		|		|
|| **delete task** | | |
|1| delete task `Write chinese essay` | delete 45 | Task 45 is deleted |
|2| delete task with invalid index | delete 1000 | Error message will be shown |
|3| delete task with alias command remove | remove 47 | Task 47 is deleted |
|Undo|		|		|		|
...
|| | edit 48 d/buy oranges | Task description is edited |
...
|1| find tasks with keyword `school` | find school | Shows 12 tasks with `school` in all tabs |
|| | list | Clears the filter and shows all tasks |
|2| find tasks with keyword `school` or `home` | find school home | Shows 13 tasks with `school` or `home` in all tabs |
|| | list | Clears the filter and shows all tasks |
|3| find tasks with partial keyword `sch` | find sch | Show 12 tasks with `sch` in all tabs|
|| | list | Clears the filter and shows all tasks |
|4| find tasks with typo in keyword | find prisentation | Shows 1 task with `presentation` in `To Do` |
|| | list | Clears the filter and shows all tasks |
||		|		|		|
|| **find by date**<br><br> Search for tasks that contain the specified date point or date range | | |
|1| find tasks on 10 april 2017 | findbydate 10 apr 2017 | 15 tasks listed with start dates <= 10 apr 2017 or end dates >= 10 apr 2017 |
|1| | list | Clears the filter and shows all tasks |
|2| find tasks from 10-11 april 2017 | findbydate 10 apr 2017 to 11 apr 2017 | 3 tasks listed with start dates <= 10 apr 2017 and end dates >= 11 apr 2017 |
|| | list | Clears the filter and shows all tasks |
|Save|		|		|		|
